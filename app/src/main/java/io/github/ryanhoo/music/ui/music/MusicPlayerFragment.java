...
public class MusicPlayerFragment extends BaseFragment implements MusicPlayerContract.View, IPlayback.Callback {
    ...
    public void onToEdit(View view) {
        if (mPlayer == null)
            Toast.makeText(getActivity(), "请先选择音频", Toast.LENGTH_SHORT).show();
        if (mPlayer.isPlaying()) {
            mPlayer.pause();
        }
        RadioButton jump=getActivity().findViewById(R.id.radio_button_settings);
        jump.setChecked(true);
        Toast.makeText(getActivity(),"成功跳转", Toast.LENGTH_LONG);//可以啊
